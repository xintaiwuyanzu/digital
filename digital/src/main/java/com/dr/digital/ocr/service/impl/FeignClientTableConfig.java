 ...
 */
@Configuration
public class FeignClientTableConfig implements RequestInterceptor {
    Logger logger = LoggerFactory.getLogger(FeignClientTableConfig.class);
    @Autowired
    OcrConfig ocrConfig;
    ...
    public void apply(RequestTemplate template) {
        ...
        template.header("Api-Auth-pubkey", ocrConfig.getTable_pubkey());
        ...
        logger.info("table-header:" + template.headers());
    }
    ...
    String getSign(String nonce, String timestamp) {
        //生成随机[a-z]字符串，包含大小写
        try {
            return shaEncode(nonce + timestamp + ocrConfig.getTable_secret_key());
        } catch (Exception e) {
            e.printStackTrace();
            return "获取签名失败";
        }
