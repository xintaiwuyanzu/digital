 ...
 */
@Configuration
public class FeignClientGeneralConfig implements RequestInterceptor {
    Logger logger = LoggerFactory.getLogger(FeignClientGeneralConfig.class);
    @Autowired
    OcrConfig ocrConfig;
    ...
    public void apply(RequestTemplate template) {
        ...
        template.header("Api-Auth-pubkey", ocrConfig.getGeneral_pubkey());
        ...
        logger.info("General-header:" + template.headers());
    }
    ...
    String getSign(String nonce, String timestamp) {
        //生成随机[a-z]字符串，包含大小写
        try {
            return shaEncode(nonce + timestamp + ocrConfig.getGeneral_secret_key());
        } catch (Exception e) {
            e.printStackTrace();
            return "获取签名失败";
        }
