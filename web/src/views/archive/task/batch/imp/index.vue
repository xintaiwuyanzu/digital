        ...
        <div class="index_main" v-loading="loading">
            ...
            <el-dialog width="50%" title="检查报告" :visible.sync="dialogShow" append-to-body>
                <el-table :data="data1" border class="table-container">
                    ...
                    <el-table-column prop="archiveCode" label="档号" show-overflow-tooltip align="center"/>
    ...
    export default {
        ...
        methods: {
            ...
            async loadData(params) {
                this.loading = true
                const {data} = await this.$post('/batch/page', {batchType: 'IMP', ...params})
            ...
            remove(id) {
                this.$confirm('此操作将删除选中数据, 是否继续?', '提示', {
                    ...
                    if (ids.length > 0) {
                        this.loading = true
                        this.$http.post('/batch/delete', {id: ids.join(','), type: "IMP"})
                            .then(({data}) => {
                                if (data.success) {
            ...
            async download(row) {
                if (row.fileLocation) {
                    window.open("/api/download/getUploadDownLoadPath?fileFullPath=" + encodeURI(row.fileLocation));
                }
            },
            detail(id) {
                this.$router.push({path: '/archive/task/batch/imp/detail', query: {batchId: id}})
            },
            $init() {
                this.loadData()
            },
            ...
            showDialog(row) {
                ...
                this.loading = true
                this.$http.post('/testrecord/batchDetail', {
                    id: row.id,
                    batchType: 'IMP',
